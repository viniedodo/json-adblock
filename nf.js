...
function requestDoubanRating(imdbId) {
    return new Promise(function (resolve, reject) {
        ...
        $task.fetch(db).then(response => {
                ...
                resolve({ rating: "Douban:  " + errorTip().reason.error });
     });
    });
}
...
function get_IMDb_message(data) {
    ...
    if (ratings.length > 0) {
        ...
        if (imdb_source == "Internet Movie Database") {
            ...
            rating_message = "IMDb评分:  " + imdb_rating.replace(/\/10/, "") + "     " + "IMDb评价:  " + imdb_votes;
            if (data.Type == "movie") {
                if (ratings.length > 1) {
                    const source = ratings[1]["Source"];
                    if (source == "Rotten Tomatoes") {
                        const tomatoes = ratings[1]["Value"];
                        tomatoes_message = "烂番茄指数: " + tomatoes;
                    }
                }
            }
        }
...
// ${average.length > 0 ? average + "/10" : "N/A"}
function get_douban_rating_message(data) {
    const average = data.rating.average;
    const numRaters = data.rating.numRaters;
    const rating_message = `豆瓣评分:  ${average.length > 0 ? average + "" : "N/A"}     豆瓣评价:  ${numRaters == 0 ? "" : parseFloat(numRaters).toLocaleString()}`;
